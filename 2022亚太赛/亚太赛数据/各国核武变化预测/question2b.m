...
data = CountrysStocks(TotalContryNum+1,:)';  %%取最后一列，也就是世界总体的变化
perVal = 24;
perLayer = 12;
...
for j=1:PreYear             %%这里的话，则是求
    ...
    end     %%将国家往期核武器拥有量、全球整体核武器数量以及该国核武器占比纳入考虑范围，给定输入参数
...
function [p,p2] = MyPerdict(net,data,perVal,PreYear)%%这个是总的预测
    ...
    for i=1:len-perVal     %依次用perVal个准确的值预测下一个
        ...
        p(i+perVal) = temp1{1};      
        p2(i+perVal) = round(p(i+perVal)*0.15 + data(i+perVal)*0.85);   %%将本国与世界纳入考虑范围，给定输入参数
    end
    %p2 = round(p);
    if PreYear>0
        for i=1:PreYear
