...
def get_answer(r,y,b,samp):
    ...
    price=(60*r+65*y+63*b)*2
    ans=ans.append({"red":r,"yellow":y,"blue":b,"color_difference":delta_e,"price":price},ignore_index=True)
    return delta_e

def func(args):
    global sample,inf_samp
    fun=lambda x:(x[0]+x[1]+x[2])*2
    return fun
...
def con(args):      ###让k/s进行匹配
    ...
    cons+=({"type":"ineq","fun":lambda x:1-get_answer(x[0],x[1],x[2],sample)},)   ###色差<1
    return cons

args=()
args1=()
for sample in range(1,6): ##单单抽出每个样本来求
    Ks_target=np.array(Ks_samp[sample])
    x0=np.array([0 for j in range(3)])
    cons=con(args1)
    ans=pd.DataFrame(columns=["red","yellow","blue","color_difference","price"])
    res=minimize(func(args),x0,constraints=cons)
